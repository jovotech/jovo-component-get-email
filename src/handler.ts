  ...
  if (this.isAlexaSkill()) {
    ...
    if (config.type === 'account-linking') {
      if (!token) {
        ...
        // @ts-ignore
        this.$alexaSkill.showAccountLinkingCard().tell(this.$speech);
      } else {
      ...
      try {
        ...
        if (error.code === 'ACCESS_DENIED' || error.code === 'NO_USER_PERMISSION') {
          ...
          // @ts-ignore
          this.$alexaSkill.showAskForContactPermissionCard('email').tell(this.$speech);
        } else {
...
async function accountLinkingGoogleAssistant(
